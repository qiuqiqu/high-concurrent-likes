...
public class SyncThumb2DBCompensatoryJob {  
    ...
    public void run() {
        ...
        String prefix = ThumbConstant.TEMP_THUMB_KEY_PREFIX.replace("%s", "");
        thumbKeys.stream()
                .filter(ObjUtil::isNotNull)
                .forEach(thumbKey -> {
                    if (thumbKey.startsWith(prefix)) {
                        needHandleDataSet.add(thumbKey.substring(prefix.length()));
                    } else {
                        log.warn("Unexpected thumbKey format: {}", thumbKey);
                    }
                });
